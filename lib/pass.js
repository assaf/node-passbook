...
Pass.prototype.pipe = function(output) {
  ...
  var pass = this
  async.forEach(Object.keys(pass.images), function (key, cb) {
    var filename = key.replace(/2x$/, "@2x") + ".png";
    writeFile(addFile(filename), pass.images[key], cb);
  }, function(err) {
    if (err) {
      lastError = err;
      doneWithWriting();
      ...
      pass.localizations
      async.forEach(Object.keys(pass.localizations), function (key, cb) {
        writeFile(addFile(key), pass.localizations[key], cb);
      }, function(err) {
