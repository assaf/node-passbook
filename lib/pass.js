...
function signManifest(template, manifest, callback) {
  ...
  var sign = execFile("openssl", args, { stdio: "pipe" }, function(error, stdout, stderr) {    
    ...
    if (error || trimmedStderr.length != 0 && trimmedStderr != "Loading 'screen' into random state - done" && trimmedStderr != "Loading 'screen' into random state - done\r\nunable to write 'random state'") {
